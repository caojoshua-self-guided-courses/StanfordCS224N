...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden_weight = nn.Parameter(nn.init.xavier_uniform_(torch.empty(n_features * self.embed_size, self.hidden_size)))
        self.embed_to_hidden_bias = nn.Parameter(nn.init.uniform_(torch.empty(hidden_size)))
        self.dropout = nn.Dropout(dropout_prob)
        self.hidden_to_logits_weight = nn.Parameter(nn.init.xavier_uniform_(torch.empty(self.hidden_size, n_classes)))
    ...
    def embedding_lookup(self, w):
        ...
        x = torch.flatten(torch.index_select(self.embeddings, 0, torch.flatten(w)))
    ...
    def forward(self, w):
        ...
        hidden = torch.add(torch.matmul(w, self.embed_to_hidden_weight), self.embed_to_hidden_bias)
        relu = torch.nn.functional.relu(hidden)
        dropout = self.dropout(relu)
        logits = torch.add(torch.matmul(dropout, self.hidden_to_logits_weight), self.hidden_to_logits_bias)
